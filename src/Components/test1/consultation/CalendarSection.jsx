  ...
  //       "freeConsultanceData",
  //       JSON.stringify({ ...stored, free_consultance_date: onlyDate.toISOString() })
  //     );
  //   };
...
const CalendarSection = ({ onDateSelect, onTimeZoneChange }) => {
  const after24hours = dayjs().add(1, "day");

  const [selectedDate, setSelectedDate] = useState(null);
  const [timeZone, setTimeZone] = useState(null);

  // ✅ GMT offset helper (same)
  const getGMTOffset = (tz) => {
    const offsetMinutes = dayjs().tz(tz).utcOffset();
    const sign = offsetMinutes >= 0 ? "+" : "-";
    const abs = Math.abs(offsetMinutes);
  ...
  useEffect(() => {
    ...
    if (free_consultance_date) {
      const parsedDate = dayjs(free_consultance_date);
      setSelectedDate(parsedDate);
      onDateSelect(parsedDate.toDate());
    }

    if (time_zone) {
      const tzObj = { value: time_zone, label: time_zone };
      setTimeZone(tzObj);
      onTimeZoneChange(time_zone);
    }
  }, []);
  ...
  const handleDateChange = (date) => {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        ...stored,
        free_consultance_date: jsDate.toISOString(),
      })
    );
  };

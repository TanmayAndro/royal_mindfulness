...
export const Payment = () => {
  ...
  const checkExistingBooking = async () => {
    try {
      ...
      const res = await axios.get(
        `https://be-royal-mindfulness.onrender.com/trainer_bookings/check_booking?from_date=${sessionDate}&from_time=${sessionTime}`,
        { headers }
      );
  ...
  const bookTrainer = async () => {
    try {
      const headers = { token: token.trim() };
      const res = await axios.post(
        "https://be-royal-mindfulness.onrender.com/trainer_bookings",
        payload,
        { headers }
      );
